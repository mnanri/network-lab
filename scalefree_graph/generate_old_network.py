...
def generate_umbalance_network(n1=100, n2=25, m=2, graph_num=4, large_graph_num=2, link_level=20):
  G = []
  ...
  # Labeling Order with Degree Centrality / Closeness Centrality / Betweenness Centrality / PageRank
  ...
  for i in range(n1):
    ...
    for j in range(large_graph_num):
      ...
      # tmp.append(sorted(dict(nx.betweenness_centrality(G[j])).items(), key=lambda x: x[1], reverse=True)[i][0] + j*n1)
  ...
  for i in range(n2):
    ...
    for j in range(small_graph_num):
      ...
      # tmp.append(sorted(dict(nx.betweenness_centrality(G[j+large_graph_num])).items(), key=lambda x: x[1], reverse=True)[i][0] + j*n2 + large_graph_num*n1)
  ...
  return H, large_graph_cent_order_list, small_graph_cent_order_list
