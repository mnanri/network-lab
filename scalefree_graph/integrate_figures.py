...
def show_integarte_figures(tk=2, n=400, links=10, samples=20, tp=""):
  ...
  ax.plot([float(r[0][i]) for i in range(n)], [float(r[1][i]) for i in range(n)], label="(a)")
  ...
  with open(f'./scalefree_graph/task{tk}_data/task{tk}_n{n}_{links}perLink_{samples}samples_dg{tp}.csv', 'r') as f:
    ...
    # print(dg)
  ax.plot([float(dg[0][i]) for i in range(n)], [float(dg[1][i]) for i in range(n)], label="(j)")
  ...
  ax.plot([float(cc[0][i]) for i in range(n)], [float(cc[1][i]) for i in range(n)], label="(k)")
  ...
  ax.plot([float(bc[0][i]) for i in range(n)], [float(bc[1][i]) for i in range(n)], label="(l)")
  ...
  ax.plot([float(pr[0][i]) for i in range(n)], [float(pr[1][i]) for i in range(n)], label="(m)")
...
show_integarte_figures(2, 400, 10, 20, "asc")
